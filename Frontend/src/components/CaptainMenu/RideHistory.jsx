...
const RideHistory = (props) => {
  ...
  const rides = props.rideHistoryData[0].rideHistory.map(ride => ({
    id: ride.rideID,
    fname: ride.UserFirstName,
    lname: ride.UserLastName,
    mobile: ride.UserMobileNumber,
    UserImage: ride.UserProfilePicture ||'https://media.istockphoto.com/id/1300845620/vector/user-icon-flat-isolated-on-white-background-user-symbol-vector-illustration.jpg?s=612x612&w=0&k=20&c=yBeyba0hUkh14_jgv1OKqIH0CCSWU_4ckRkAoy2p73o=',
        ...
        <div className="space-y-2 ">
          {rides.map((ride) => (
            <div key={ride.id} className="border rounded-xl overflow-hidden ">
              {/* Ride Summary */}
              <div
                className="p-4 bg-yellow-50 cursor-pointer "
                onClick={() => setExpandedRide(expandedRide === ride.id ? null : ride.id)}
              >
                <div className="flex items-center justify-between bg-gray-50 active:bg-gray-300  p-2 rounded-lg">
